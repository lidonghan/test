...
static int rk_camera_scale_crop_arm(struct work_struct *work)
{
    ...
    for(y = 0; y<dstH; y++ ) {   
        ...
        for(x = 0; x<dstW; x++ ) {
            ...
            a = (psY[sY*srcW + sX]<<2);
            b = (psY[sY*srcW + sX + 1]<<2);
            c = (psY[(sY+1)*srcW + sX]<<2);
            d = (psY[(sY+1)*srcW + sX + 1]<<2);
        #else  
	        a = psY[sY*srcW + sX];
            b = psY[sY*srcW + sX + 1];
            c = psY[(sY+1)*srcW + sX];
    ...
    for(y = 0; y<dstH; y++ ) {
        ...
        for(x = 0; x<dstW; x++ ) {
            ...
            //U
			#ifdef CONFIG_SOC_RK3028
            a = (psUV[(sY*srcW + sX)*2]<<2);
            b = (psUV[(sY*srcW + sX + 1)*2]<<2);
            c = (psUV[((sY+1)*srcW + sX)*2]<<2);
            d = (psUV[((sY+1)*srcW + sX + 1)*2]<<2);
			#else
			a = psUV[(sY*srcW + sX)*2];
            ...
            //V
			#ifdef CONFIG_SOC_RK3028
            a = (psUV[(sY*srcW + sX)*2 + 1]<<2);
            b = (psUV[(sY*srcW + sX + 1)*2 + 1]<<2);
            c = (psUV[((sY+1)*srcW + sX)*2 + 1]<<2);
            d = (psUV[((sY+1)*srcW + sX + 1)*2 + 1]<<2);
			#else
