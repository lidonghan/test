...
static int batt_table[2*BATT_NUM+6] =
{
	...
	//3540, 3785, 3842, 3861, 3915, 3980, 4041, 4135, 4169, 4175, 4185	  //charge
        //                         3700/3800     3810/3920       
	    3495, 3570, 3630, 3700, 3740, 3790, 3825, 3865, 3920, 3980, 4050,  //discharge
	    3600, 3700, 3760, 3810, 3870, 3910, 3960, 4020, 4080, 4130, 4180	  //charge
	//3540, 3785, 3842, 3861, 3915, 3980, 4041, 4135, 4169, 4175, 4185	  //charge
};
...
//#endif
	...
	//chargeing state
		bat  ->pdata->is_reboot_charging = board_boot_mode();
		if( bat  ->pdata->is_reboot_charging == 1){
			printk("is_reboot_chargeing,new=%d,old=%d,bat_capacity=%d\n",new_capacity,old_capacity,bat ->bat_capacity);
			if(old_capacity < 10){
				bat ->bat_capacity = (old_capacity < 10) ?(old_capacity+2):old_capacity;
			}else{
				bat ->bat_capacity = (new_capacity-50 > old_capacity) ? new_capacity : old_capacity;
			}

		}
		else{
			printk("not_reboot_chargeing,new=%d,old=%d,bat_capacity=%d\n",new_capacity,old_capacity,bat ->bat_capacity);
			bat ->bat_capacity = (new_capacity-70 > old_capacity) ? new_capacity : old_capacity;	
		}
	}else{
		printk("not_chargeint,new=%d,old=%d,bat_capacity=%d\n",new_capacity,old_capacity,bat ->bat_capacity);
		if(new_capacity > old_capacity + 50 )
			bat ->bat_capacity = new_capacity;
		else
			bat ->bat_capacity = (new_capacity < old_capacity-20) ? new_capacity : old_capacity;  //avoid the value of capacity increase 
		if(bat->bat_capacity == 100)
			bat->bat_capacity = 99;
		if(bat->bat_capacity == 0)
			bat->bat_capacity =1;
	}
	printk("old_capacity = %d , new_capacity =%d, bat->bat_capacity=%d,bat ->bat_status=%d\n  ",old_capacity,new_capacity,bat->bat_capacity,bat ->bat_status);
...
//	struct rk30_adc_battery_data *bat = gBatteryData;
	...
	if( 1 == level ){
	//chargeing state
		bat->bat_capacity = (new_capacity-30 > old_capacity) ? new_capacity : old_capacity;
	}
	else{
		bat->bat_capacity = (new_capacity < old_capacity-20) ? new_capacity : old_capacity;  // aviod the value of capacity increase    dicharge
	}
...
extern int in_rtc_auto_wakeup_mode;
...
struct rk30_adc_battery_data  *bat = container_of((work), \
	...
	if( 0 == bat ->pdata ->charging_sleep){
		if(( 1 == bat->charge_level)&&(in_rtc_auto_wakeup_mode != 1)){  // charge
